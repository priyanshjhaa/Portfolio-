...
export default function Projects({ active = false, onOpenProject }: ProjectsProps) {
  ...
  const renderCard = (project: typeof projects[number], featured = false) => (
    <article
      ...
      className={cn(
        ...
        featured && 'shadow-[0_28px_70px_-48px_rgba(245,158,11,0.3)]'
      )}
    >
      {project.image && (
        <PointerSurface className="overflow-hidden rounded-[20px] border border-white/8 bg-black/40">
          <button
            type="button"
  ...
  return (
    <section className="section-shell relative overflow-hidden px-4 py-20 md:py-24">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_left,rgba(245,158,11,0.08),transparent_24%)] pointer-events-none" />
